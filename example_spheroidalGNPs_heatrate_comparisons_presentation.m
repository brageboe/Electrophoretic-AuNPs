...
% System
freq = 10e6;
A = linspace(3, 100, 1e3).*1e-9;  % length of one gnp axis (diameter or length)
...
% % OBLATES
% subplot(1,3,3)
% semilogy(A/1e-9, heatrate_oblate, ':', 'Color', [0 0.4470 0.7410])
% hold on
% title("Oblates")
% xlabel("Diameter [nm]")
% ylim([min_heatrate max_heatrate])
...
% % PROLATES
% subplot(1,3,2)
% semilogy(A/1e-9, heatrate_prolate, '--', 'Color', [0 0.4470 0.7410])
% hold on
% title("Prolates")
% xlabel("Length [nm]")
% ylim([min_heatrate max_heatrate])
...
% hold off
% 
legend("Spheres", 'location','northeastoutside')
% legend("Spheres", "Oblates", 'location','northeastoutside')
% legend("Spheres", "Oblates", "Prolates", 'location','northeastoutside')
hold off
...
axesHandles.Legend.FontSize = 10;

f.Position = [680 458 340 280]; % [x0 y0 width height]
